...
class NetworkLayer:
    """The network layer that delivers packets and acknowledgments between sender and receiver"""
    ...
    def __init__(self, reliability=1.0, delay=1.0, pkt_corrupt=True, ack_corrupt=True):
        ...
        :param delay: the round trip time for sending a packet and receiving a reply
    ...
    def get_network_reliability(self):
        ...
        :return: a float number representing the current network reliability
        """
        return self.reliability

    def __packet_corruption_probability(self):
        """calculate the probability that a packet will be corrupted
        :return: True if the probability is greater than the network reliability
        """
    ...
    def udt_send(self, frame):
        ...
        :param frame: a Python dictionary representing a sender's packet or a receiver's reply
        :return: the receiver's reply as a Python dictionary returned to the sender
        """

        # TODO: You may add ONLY print statements to this function for debugging purposes
        self.packet = frame
        s_test = self.__packet_corruption_probability()

        if s_test and self.pkt_corrupt:
            self.__corrupt_packet()
            print(f"\033[91mNetwork Layer: corruption occurred \033[0m{self.packet}")
        ...
        if r_test and self.ack_corrupt:
            ...
            print(f"\033[91mNetwork Layer: corruption occurred \033[0m{self.reply}")
